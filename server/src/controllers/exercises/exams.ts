...
class ExamsController {
  ...
  public get = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      conditions.$expr = {
        $and: [
          { $eq: ["$exercise", req.query.exercise] },
          { $eq: ["$startBy", req.verify._id] },
        ],
      };
      const countDocuments = await MExams.where(
        conditions as any
      ).countDocuments();
  ...
  public getQuestions = async (
    ...
    try {
      ...
      const exercise = await MExercises.findById(
        { _id: req.query.key },
        "questions"
      );
      ...
      const countDocuments = await MQuestion.where(
        conditions as any
      ).countDocuments();
  ...
  public put = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      for await (const e of questions) {
        const item = req.body.questions.find(
      ...
      // Push logs
      Logger.set(req, this.path, req.body._id, "update");
  ...
  public patch = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      for await (const _id of req.body._id) {
        ...
        if (x) {
          const _x = await MExams.updateOne(
            { _id },
  ...
  public delete = async (req: Request, res: Response, next: NextFunction) => {
    try {
      if (Types.ObjectId.isValid(req.params._id)) {
        MExams.deleteOne({ _id: req.params._id }, undefined, (e: any) => {
          if (e) return res.status(500).send(e);
          // Push logs
