...
class CategoriesController {
  ...
  public get = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const countDocuments = await MCategory.where(conditions as any).countDocuments();
      const options = {
  ...
  public getAttr = async (req: Request, res: Response, next: NextFunction) => {
    try {
      MCategory.distinct(req.query.key ? 'attr.key' : 'attr.value', null, (e, rs) => {
        ...
        if (req.query.page && req.query.rowsPerPage) rs = Pagination.get(rs, page, rowsPerPage);
        return res.status(200).json({ rowsNumber: countDocuments, data: rs });
      });
    } catch (e) {
      return res.status(500).send('invalid');
